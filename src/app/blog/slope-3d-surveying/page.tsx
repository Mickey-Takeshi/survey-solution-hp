...
const faqJsonLd = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        "@type": "Answer",
        text: "問題なく対応できます。3Dレーザースキャナは非接触計測のため、人が立ち入れない急勾配（45度以上）の法面でも安全にデータを取れます。スキャナの設置位置を法尻や対岸に変えることで、死角を減らしながら法面全体をカバーします。",
      },
    },
    {
      "@type": "Question",
      ...
      acceptedAnswer: {
        "@type": "Answer",
        text: "比較できます。TS測量の座標データを3Dレーザーの点群データに重ね合わせれば、同一座標系上で差分解析が可能です。過去のTS測量データとの経年比較にも対応しているため、既存データを活かしたまま3D管理へ移行できます。",
      },
    },
  ],
};
        ...
        <div className="max-w-[800px] mx-auto px-6">
          ...
          <div className="bg-primary/5 border-l-4 border-primary p-6 rounded-r-lg mb-10">
            <p className="text-base leading-loose text-gray-700">
              <strong>法面測量に3Dレーザースキャンを活用すると、非接触で安全に計測でき、面的データにより変位を±5mmの精度で管理できます。</strong>従来のTS測量による断面計測と比べて計測時間は1/3。変状の見落としリスクも大幅に減らせます。ここでは、法面測量の課題から3Dレーザーの仕組み、導入メリット、注意点、ドローンとの使い分けまで、現場目線でまとめました。
            </p>
          </div>
          ...
          <div className="prose prose-gray max-w-none">
            ...
            <p className="text-base leading-loose text-gray-700 mb-4">
              計測の流れはシンプルです。まず、法面周辺に基準点（ターゲット）を設置し、既知点との座標整合を行います。次に、3Dレーザースキャナを1〜3箇所に据えて法面を360度スキャンします。1回のスキャンで数百万〜数千万点の点群データを取得でき、点間隔は計測距離50mで約5mmと非常に高密度です。
            </p>
            <p className="text-base leading-loose text-gray-700 mb-4">
            ...
            <p className="text-base leading-loose text-gray-700 mb-6">
              3Dレーザースキャンを法面測量に取り入れると、従来手法では難しかった4つの強みが活きてきます。
            </p>
            <div className="space-y-4 mb-8">
              {[
                {
            ...
            <p className="text-base leading-loose text-gray-700 mb-6">
              メリットの多い3Dレーザー測量ですが、現場条件によっては気をつけたい点もあります。
            </p>
            <div className="space-y-4 mb-8">
              {[
                {
                  ...
                  desc: "草木が繁茂している法面では、レーザーが植生表面に反射し、地表面データを直接取得できないことがあります。点群処理ソフトウェアで植生と地表面を分離するフィルタリングが必要です。密な植生だと精度が落ちるため、伐採後の計測が望ましいケースもあります。計測前の現地踏査で植生状況を把握しておくのが鉄則です。",
                },
                {
                  num: "2",
            ...
            <p className="text-base leading-loose text-gray-700 mb-6">
              TS測量と3Dレーザー測量、どこがどう変わるのか比較してみましょう。手法選定の参考にしてください。
            </p>
            ...
            <div className="space-y-4 mb-8">
              {[
                ...
                },
                {
                  ...
                  a: "問題なく対応できます。3Dレーザースキャナは非接触計測のため、人が立ち入れない急勾配（45度以上）の法面でも安全にデータを取れます。スキャナの設置位置を法尻や対岸に変えることで、死角を減らしながら法面全体をカバーします。",
                },
                {
                  q: "既存のTS測量データと3Dレーザーのデータは比較できますか？",
                  a: "比較できます。TS測量の座標データを3Dレーザーの点群データに重ね合わせれば、同一座標系上で差分解析が可能です。過去のTS測量データとの経年比較にも対応しているため、既存データを活かしたまま3D管理へ移行できます。",
                },
              ].map((item) => (
            ...
            <p className="text-base leading-loose text-gray-700 mb-4">
              法面測量に3Dレーザースキャンを導入すれば、作業員の安全確保と計測精度の向上を両立できます。面的データによる変位管理は、従来のTS断面計測では難しかった法面全体の状態把握を実現してくれます。
            </p>
            <div className="bg-gray-50 p-5 rounded-lg mb-6">
            ...
            <p className="text-base leading-loose text-gray-700 mb-4">
              法面の維持管理や変状モニタリングに3Dレーザー測量を検討されている方は、まず現場の概要をお聞かせください。法面の規模・勾配・植生条件に応じて、最適な計測手法と費用感をご案内します。
            </p>
          </div>
